(function(jQuery){jQuery.jCryption=function(el,options){var base=this;base.$el=$(el);base.el=el;base.$el.data("jCryption",base);base.init=function(){base.options=jQuery.extend({},jQuery.jCryption.defaultOptions,options);$encryptedElement=jQuery("<input />",{type:'hidden',name:base.options.postVariable});if(base.options.submitElement!==false){var $submitElement=base.options.submitElement;}else{var $submitElement=base.$el.find(":input:submit");}
$submitElement.bind(base.options.submitEvent,function(){jQuery(this).attr("disabled",true);if(base.options.beforeEncryption()){jQuery.jCryption.getKeys(base.options.getKeysURL,function(keys){jQuery.jCryption.encrypt(base.$el.serialize(),keys,function(encrypted){$encryptedElement.val(encrypted);jQuery(base.$el).find(":input").attr("disabled",true).end().append($encryptedElement).submit();});});}
return false;});};base.init();};jQuery.jCryption.getKeys=function(url,callback){var jCryptionKeyPair=function(encryptionExponent,modulus,maxdigits){setMaxDigits(parseInt(maxdigits,10));this.e=biFromHex(encryptionExponent);this.m=biFromHex(modulus);this.chunkSize=2*biHighIndex(this.m);this.radix=16;this.barrett=new BarrettMu(this.m);};jQuery.getJSON(url,function(data){keys=new jCryptionKeyPair(data.e,data.n,data.maxdigits);if(jQuery.isFunction(callback)){callback.call(this,keys);}});};jQuery.jCryption.encrypt=function(string,keyPair,callback){var charSum=0;for(var i=0;i<string.length;i++){charSum+=string.charCodeAt(i);}
